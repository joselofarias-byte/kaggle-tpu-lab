"""
EXPERIMENTAL BRANCH: vllm-tpu 0.29.0. Not the production baseline.
See docs/EXPERIMENTAL_VLLM_029.md before serving this on a Kaggle TPU.
...
DEFAULTS = {
    ...
    "max_num_seqs": 4,
    "mtp_tokens": 3,               # MTP spec decoding. Stock vllm-tpu
                                   # 0.29.0 still needs a GDN state rollback;
                                   # we apply patches/mtp-rollback-v0290.diff
                                   # (a re-port of upstream PR #3178). EXPERIMENTAL:
                                   # not re-measured on a Kaggle TPU. See
                                   # docs/EXPERIMENTAL_VLLM_029.md.
...
def server_args(cfg):
    ...
    # EXPERIMENTAL, vllm-tpu 0.29.0 only. k0valik enables this because upstream
    # #3422 fixed hybrid-GDN prefix caching and the platform disables it under
    # spec-decode. Not validated on a Kaggle TPU in this repo. See
    # docs/EXPERIMENTAL_VLLM_029.md.
