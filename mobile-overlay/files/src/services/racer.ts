  ...
  private handleNtfyEvent(session: RaceSession, ev: NtfyEvent, text: string, modelName: string) {
    ...
    if (verdict.kind === 'mount') {
      ...
      const ep: LiveEndpoint = {
        ...
        modelId: ev.model_id || session.endpoint?.modelId,
        displayName: ev.display_name || session.endpoint?.displayName,
        backend: ev.backend || session.endpoint?.backend,
        accelerator: ev.accelerator || session.endpoint?.accelerator,
        status: 'READY',
